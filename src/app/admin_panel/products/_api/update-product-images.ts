...
const updateProduct = ({ id, formData }: UpdateProductOptions, token: string): Promise<void> => {
    ...
    return createData<FormData, void>(`admin/products/${id}/images-update?_method=PUT`, formData, headers);
};
...
export const useUpdateProductImages = () => {
    ...
    const { mutate, isLoading } = useMutation({
        ...
        onSuccess: async (_, variables) => {
                    ...
                    const updatedProducts = products.map(product => {
                        if (product.id === variables.id) {
                            ...
                            if (object.primary_image) {
                                updatedProduct.primary_image = URL.createObjectURL(object.primary_image);
                            }
            ...
            queryClient.invalidateQueries(['productsImages', variables.id]);
        },
    });
